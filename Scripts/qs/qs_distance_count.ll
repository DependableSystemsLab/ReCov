; ModuleID = 'qs_distance_count.c'
target datalayout = "e-p:64:64:64-i1:8:8-i8:8:8-i16:16:16-i32:32:32-i64:64:64-f32:32:32-f64:64:64-v64:64:64-v128:128:128-a0:0:64-s0:64:64-f80:128:128-f128:128:128-n8:16:32:64"
target triple = "x86_64-unknown-linux-gnu"

%struct.FILE = type { i32, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, i8*, %struct._IO_marker*, %struct.FILE*, i32, i32, i64, i16, i8, [1 x i8], i8*, i64, i8*, i8*, i8*, i8*, i64, i32, [20 x i8] }
%struct._IO_marker = type { %struct._IO_marker*, %struct.FILE*, i32 }

@fi_cycle_counter = unnamed_addr global i64 0
@cycleInterval = unnamed_addr global i64 1
@cycleFactor = unnamed_addr global i64 1
@bbCounter = unnamed_addr global i64 0
@qsCounter = unnamed_addr global i64 0
@maxQs = unnamed_addr global i64 1
@minQs = unnamed_addr global i64 1
@totalQs = unnamed_addr global i64 0
@qsNumber = unnamed_addr global i64 0
@preciseCounter = unnamed_addr global i64 0
@lastQsCounter = unnamed_addr global i64 0
@.str = private unnamed_addr constant [23 x i8] c"crashcount_latency.txt\00", align 1
@.str1 = private unnamed_addr constant [2 x i8] c"w\00", align 1
@.str2 = private unnamed_addr constant [5 x i8] c"%lld\00", align 1
@.str3 = private unnamed_addr constant [42 x i8] c"\0A\0A\0A***********\0AAverage QS Distance: %lld\0A\00", align 8
@.str4 = private unnamed_addr constant [47 x i8] c"Max/Min Qs Distance: %lld, %lld\0A***********\0A\0A\0A\00", align 8

define void @latencyCount(i64 %llfiIndex) nounwind {
entry:
  %llfiIndex_addr = alloca i64, align 8
  %f = alloca %struct.FILE*
  %"alloca point" = bitcast i32 0 to i32
  store i64 %llfiIndex, i64* %llfiIndex_addr
  %0 = load i64* @preciseCounter, align 8
  %1 = add nsw i64 %0, 1
  store i64 %1, i64* @preciseCounter, align 8
  %2 = load i64* @fi_cycle_counter, align 8
  %3 = add nsw i64 %2, 1
  store i64 %3, i64* @fi_cycle_counter, align 8
  %4 = load i64* @cycleInterval, align 8
  %5 = mul nsw i64 %4, 10
  %6 = load i64* @fi_cycle_counter, align 8
  %7 = icmp sle i64 %5, %6
  br i1 %7, label %bb, label %bb1

bb:                                               ; preds = %entry
  %8 = load i64* @cycleInterval, align 8
  %9 = mul nsw i64 %8, 10
  store i64 %9, i64* @cycleInterval, align 8
  store i64 1, i64* @cycleFactor, align 8
  br label %bb1

bb1:                                              ; preds = %bb, %entry
  %10 = load i64* @cycleInterval, align 8
  %11 = load i64* @cycleFactor, align 8
  %12 = mul nsw i64 %10, %11
  %13 = load i64* @fi_cycle_counter, align 8
  %14 = icmp slt i64 %12, %13
  br i1 %14, label %bb2, label %bb3

bb2:                                              ; preds = %bb1
  %15 = load i64* @cycleFactor, align 8
  %16 = add nsw i64 %15, 1
  store i64 %16, i64* @cycleFactor, align 8
  %17 = call %struct.FILE* @fopen(i8* noalias getelementptr inbounds ([23 x i8]* @.str, i64 0, i64 0), i8* noalias getelementptr inbounds ([2 x i8]* @.str1, i64 0, i64 0)) nounwind
  store %struct.FILE* %17, %struct.FILE** %f, align 8
  %18 = load i64* @fi_cycle_counter, align 8
  %19 = load %struct.FILE** %f, align 8
  %20 = call i32 (%struct.FILE*, i8*, ...)* @fprintf(%struct.FILE* noalias %19, i8* noalias getelementptr inbounds ([5 x i8]* @.str2, i64 0, i64 0), i64 %18) nounwind
  %21 = load %struct.FILE** %f, align 8
  %22 = call i32 @fclose(%struct.FILE* %21) nounwind
  br label %bb3

bb3:                                              ; preds = %bb2, %bb1
  br label %return

return:                                           ; preds = %bb3
  ret void
}

declare %struct.FILE* @fopen(i8* noalias, i8* noalias)

declare i32 @fprintf(%struct.FILE* noalias, i8* noalias, ...) nounwind

declare i32 @fclose(%struct.FILE*)

define void @qsPlace(i64 %llfiIndex) nounwind {
entry:
  %llfiIndex_addr = alloca i64, align 8
  %"alloca point" = bitcast i32 0 to i32
  store i64 %llfiIndex, i64* %llfiIndex_addr
  %0 = load i64* @qsNumber, align 8
  %1 = add nsw i64 %0, 1
  store i64 %1, i64* @qsNumber, align 8
  %2 = load i64* @preciseCounter, align 8
  %3 = load i64* @lastQsCounter, align 8
  %4 = sub nsw i64 %2, %3
  %5 = load i64* @totalQs, align 8
  %6 = add nsw i64 %4, %5
  store i64 %6, i64* @totalQs, align 8
  %7 = load i64* @preciseCounter, align 8
  %8 = load i64* @lastQsCounter, align 8
  %9 = sub nsw i64 %7, %8
  %10 = load i64* @maxQs, align 8
  %11 = icmp sgt i64 %9, %10
  br i1 %11, label %bb, label %bb1

bb:                                               ; preds = %entry
  %12 = load i64* @preciseCounter, align 8
  %13 = load i64* @lastQsCounter, align 8
  %14 = sub nsw i64 %12, %13
  store i64 %14, i64* @maxQs, align 8
  br label %bb1

bb1:                                              ; preds = %bb, %entry
  %15 = load i64* @preciseCounter, align 8
  %16 = load i64* @lastQsCounter, align 8
  %17 = sub nsw i64 %15, %16
  %18 = load i64* @minQs, align 8
  %19 = icmp slt i64 %17, %18
  br i1 %19, label %bb2, label %bb3

bb2:                                              ; preds = %bb1
  %20 = load i64* @preciseCounter, align 8
  %21 = load i64* @lastQsCounter, align 8
  %22 = sub nsw i64 %20, %21
  store i64 %22, i64* @minQs, align 8
  br label %bb3

bb3:                                              ; preds = %bb2, %bb1
  %23 = load i64* @qsNumber, align 8
  %24 = srem i64 %23, 10000
  %25 = icmp eq i64 %24, 0
  br i1 %25, label %bb4, label %bb5

bb4:                                              ; preds = %bb3
  %26 = load i64* @totalQs, align 8
  %27 = load i64* @qsNumber, align 8
  %28 = sdiv i64 %26, %27
  %29 = call i32 (i8*, ...)* @printf(i8* noalias getelementptr inbounds ([42 x i8]* @.str3, i64 0, i64 0), i64 %28) nounwind
  %30 = load i64* @minQs, align 8
  %31 = load i64* @maxQs, align 8
  %32 = call i32 (i8*, ...)* @printf(i8* noalias getelementptr inbounds ([47 x i8]* @.str4, i64 0, i64 0), i64 %31, i64 %30) nounwind
  br label %bb5

bb5:                                              ; preds = %bb4, %bb3
  %33 = load i64* @preciseCounter, align 8
  store i64 %33, i64* @lastQsCounter, align 8
  br label %return

return:                                           ; preds = %bb5
  ret void
}

declare i32 @printf(i8* noalias, ...) nounwind
