...
namespace{
	struct bishe_insert : public ModulePass{
		...
		virtual bool runOnModule(Module &M)
		{
			...
			for(int i=0 ;i<selectedInsts.size();i++){
				...
				if(!isUsedInOtherOperand(currentInst)){
					...
					// Split current bb
					//errs() << checkPosInst->getParent() << "\n";
					BasicBlock* currentBb = checkPosInst->getParent();
					BasicBlock* continueBb = currentBb->splitBasicBlock(checkPosInst);

					// Create check block
					BasicBlock* checkBb = BasicBlock::Create(M.getContext(), "checkBb", currentBb->getParent(), continueBb);
					Instruction* checkerExitInst = BranchInst::Create(continueBb, checkBb);
					//ReturnInst::Create(M.getContext(), checkBb);
					insertExitFunction(checkerExitInst, M.getContext());


					// Insert a br instruction to original bb (after cmp)
					BasicBlock::iterator oldBrInst = cmpInst;
					oldBrInst++;
					Instruction* brCheckInst = BranchInst::Create(continueBb, checkBb, cmpInst, oldBrInst);
					oldBrInst->eraseFromParent();
				}
			}
		...
		void insertExitFunction(Instruction* instPos, LLVMContext &context){
			...
			Constant* checker_handler_c = appModule->getOrInsertFunction("checker_exit", checker_type);
